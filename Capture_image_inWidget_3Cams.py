...
def createwidgets(): 
    ...
    root.browseButton = tk.Button(root, width=18, text="Browse Save Path", command=destBrowse)
    root.browseButton.grid(row=3, column=2, padx=10, pady=10)

    root.captureBTN = tk.Button(root, text="Capture #1", command=lambda:Capture(root.cap1),  
                                font=('Times New Room',15), width=20)
    root.captureBTN.grid(row=4, column=1, padx=10, pady=10)

    root.CAMBTN = tk.Button(root, text="Stop Camera1", command=lambda:StopCAM(1),  
    ...
    root.captureBTN2 = Button(root, text="Capture #2", command=lambda:Capture(root.cap2),  
                             font=('Times New Room',15), width=20)
    root.captureBTN2.grid(row=4, column=3, padx=10, pady=10)

    root.CAMBTN2 = Button(root, text="Stop Camera2", command=lambda:StopCAM(2),  font=('Times New Room',15), width=15)
    ...
    root.captureBTN3 = Button(root, text="Capture #3", command=lambda:Capture(root.cap3),  
                             font=('Times New Room',15), width=20)
    root.captureBTN3.grid(row=8, column=1, padx=10, pady=10)

    root.CAMBTN3 = Button(root, text="Stop Camera3", command=lambda:StopCAM(3),  font=('Times New Room',15), width=15)
    ...
    root.openImageButton = Button(root, width=20, text="Browse Source Path", command=imageBrowse)
...
def StopCAM(cam_num):
    ...
    if cam_num == 2:        
        ...
        # Configuring the CAMBTN to display accordingly
        root.CAMBTN.config(text="Start Cam1", command=lambda:StartCAM( 1 ))    
        # Displaying text message in the camera label
        ...
        # Configuring the CAMBTN to display accordingly
        root.CAMBTN3.config(text="Start Cam3", command=lambda:StartCAM( 3 ))    
        # Displaying text message in the camera label
...
def StartCAM(cam_num):
    ...
    if cam_num == 2:
        ...
        # Configuring the CAMBTN to display accordingly
        root.CAMBTN2.config(text="Stop Camera2", command=lambda:StopCAM(2))
        ...
        # Configuring the CAMBTN to display accordingly
        root.CAMBTN.config(text="Stop Camera1", command=lambda:StopCAM(1))
...
curr_path = os.getcwd() 
f_json = os.path.join( curr_path, "Documents", "camera_information.json") 
f_json
